(function () {
  ...
  function renderTable(rows, month) {
    ...
    (rows || []).forEach(r => {
      ...
      addNumCell(fmtMoney(num(r.ticket_si) + num(r.ticket_no)));
      addNumCell(fmtMoney(num(r.delivery_si) + num(r.delivery_no)));
      addNumCell(fmtMoney(num(r.coupon_si) + num(r.coupon_no)));
